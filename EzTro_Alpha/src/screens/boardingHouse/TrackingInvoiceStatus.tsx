  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.filterRow}>
        ...
        <FilterColumn label="Tháng" value={selMonth === null ? "Tất cả" : `Tháng ${selMonth}`} onPress={() => setActivePicker('month')} />
        <FilterColumn label="Năm" value={selYear === null ? "Tất cả" : selYear} onPress={() => setActivePicker('year')} />
      </View>

      <View style={styles.tabContainer}>
        {["all", "unpaid", "paid"].map((t) => (
          <TouchableOpacity
            key={t} style={[styles.tab, statusFilter === t && styles.activeTab]}
        ...
        {displayInvoices.map((inv) => (
          <View key={inv._id} style={styles.card}>
            ...
            <View style={styles.totalRow}>
              ...
              <Text style={styles.totalAmountText}>{formatCurrency((inv.roomId?.rentalFee || 0) + (inv.electricityCharge || 0) + (inv.waterCharge || 0))}</Text>
            </View>
            ...
            {inv.status === 'processing' && (
              <View style={styles.actionButtonContainer}>
                <AppButton
                  ...
                  onPress={() => console.log("Confirm Payment", inv._id)}
                />
              </View>
            )}
          </View>
      ...
      <Modal visible={!!activePicker} transparent animationType="fade">
        <TouchableOpacity style={styles.modalOverlay} onPress={() => setActivePicker(null)}>
          <View style={styles.modalContent}>
            <Text style={styles.modalTitle}>Chọn {activePicker === 'house' ? 'Nhà' : activePicker === 'month' ? 'Tháng' : 'Năm'}</Text>
            <FlatList
              ...
              renderItem={({ item }) => (
                ...
                >
                  <Text style={styles.modalItemText}>
                    {activePicker === 'house' ? item.name : item === null ? "Tất cả" : (activePicker === 'month' ? `Tháng ${item}` : item)}
                  </Text>
                </TouchableOpacity>
              )}
            />
...
const styles = StyleSheet.create({
  ...
  filterRow: { flexDirection: 'row', backgroundColor: '#FFF', paddingVertical: 12, paddingHorizontal: 10, borderBottomWidth: 1, borderBottomColor: '#EEE', justifyContent: 'space-between' },
  filterCol: { flex: 1, paddingHorizontal: 5 },
  filterLabel: { fontSize: 11, color: '#999', fontWeight: 'bold', marginBottom: 4 },
  valueBox: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#F8F9FA', padding: 10, borderRadius: 8, borderWidth: 1, borderColor: '#E9ECEF' },
  ...
  card: { backgroundColor: '#FFF', borderRadius: 16, padding: 18, marginBottom: 16, elevation: 3, shadowColor: '#000', shadowOpacity: 0.1, shadowRadius: 10 },
  ...
  totalRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', borderTopWidth: 1, borderTopColor: '#F0F0F0', paddingTop: 12 },
