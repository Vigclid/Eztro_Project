...
class MailService {
     ...
     * @returns
     */
    static async generateAndSendToken(email) {
        ...
        if (!user || !pass) {
            throw new Error("SMTP credentials are missing. Please set SMTP_USER/SMTP_PASS (or SMTP_USERNAME/SMTP_PASSWORD) in Backend .env");
        }
    ...
    static buildEmailContent(token) {
        return `<!doctype html>
<html>
  <head>
    <meta charset="utf-8" />
