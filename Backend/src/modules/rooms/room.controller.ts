...
export class roomController extends GenericController<IRoom> {
  ...
  createNewRoom = async (req: Request, res: Response) => {
    try {
      ...
      return res.status(201).json(responseWrapper("success", "Tạo Phòng Thành Công", result));
    } catch (error: any) {
      ...
      if (error?.code === "HOUSE_PACKAGE_EXPIRED" || error?.message === "HOUSE_PACKAGE_EXPIRED") {
        return res
          .status(200)
          .json(
            responseWrapper(
      ...
      if (error?.code === "ROOM_LIMIT_EXCEEDED" || error?.message === "ROOM_LIMIT_EXCEEDED") {
        return res
          .status(200)
          .json(
            responseWrapper(
  ...
  updateRoom = async (req: Request, res: Response) => {
    try {
      ...
      return res.status(200).json(responseWrapper("success", "Cập Nhật Phòng Thành Công", result));
    } catch (error: any) {
  ...
  inviteTenant = async (req: Request, res: Response) => {
    try {
      ...
      if (error?.code === "INVITED_USER_NOT_TENANT") {
        return res
          .status(200)
          .json(responseWrapper("error", "Người dùng được mời không phải Tenant"));
      }
      if (error?.code === "USER_ALREADY_IN_ACTIVE_ROOM") {
        return res
          ...
          .json(
            responseWrapper(
              "error",
              "Người dùng đang ở trong phòng khác, cần rời phòng trước khi thêm"
            )
          );
      }
  ...
  joinByCode = async (req: Request, res: Response) => {
    try {
      ...
      if (error?.code === "INVALID_OR_EXPIRED_CODE") {
        return res
          .status(200)
          .json(responseWrapper("error", "Mã phòng không hợp lệ hoặc đã hết hạn"));
      }
      if (error?.code === "USER_ALREADY_IN_ACTIVE_ROOM") {
        return res
          .status(200)
          .json(responseWrapper("error", "Bạn đang ở trong phòng khác, vui lòng rời phòng trước"));
      }
      return res.status(500).json(responseWrapper("error", "Internal Server Error", error));
    }
  };
  ...
  acceptInvitation = async (req: Request, res: Response) => {
    try {
      ...
      return res
        .status(200)
        .json(responseWrapper("success", "Chấp nhận lời mời thành công", result));
    } catch (error: any) {
  ...
  getMyActiveRoom = async (req: Request, res: Response) => {
    try {
      ...
      return res
        .status(200)
        .json(responseWrapper("success", "Lấy thông tin phòng hiện tại thành công", roomInfo));
    } catch (error: any) {
      return res.status(500).json(responseWrapper("error", "Internal Server Error", error));
    }
  };
  ...
  removeRoomMember = async (req: Request, res: Response) => {
    try {
      ...
      if (error?.code === "LANDLORD_FORBIDDEN") {
        return res
          .status(403)
          .json(responseWrapper("error", "Bạn không có quyền xóa thành viên phòng này"));
      }
      if (error?.code === "ROOM_NOT_FOUND" || error?.code === "HOUSE_NOT_FOUND") {
        return res
          .status(404)
          .json(responseWrapper("error", "Không tìm thấy phòng/cụm trọ liên quan"));
      }
      return res.status(500).json(responseWrapper("error", "Internal Server Error", error));
    }
  };
